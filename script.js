...
function getExamConfig() {
  try {
    ...
    return { isOpen: false, timeLimitMinutes: 15, passThreshold: 70, maxViolations: 3, shuffleTasks: true, tasks: [] };
  }
}
...
window.initAdminPage = function () {
  ...
  function switchPanel(panel) {
    ...
    if (panel === 'sheetSync') loadSheetConfigToInputs();
  }
  ...
  function refreshExamStatusUI() {
    ...
    if (cfg.isOpen) {
      ...
      sub.textContent = 'Students can take the exam now.';
      ...
      sub.textContent = 'Students cannot take the exam.';
  ...
  $('adminSaveConfigBtn').addEventListener('click', () => {
    ...
    saveExamConfig(cfg);
    updateExamPreview();
    $('adminConfigMsg').textContent = '✅ Settings saved.';
    setTimeout(() => { $('adminConfigMsg').textContent = ''; }, 3000);
  });

  $('adminOpenExamBtn').addEventListener('click', () => {
    const cfg = getExamConfig();
    if (cfg.tasks.length === 0) {
    ...
    saveExamConfig(cfg);
    refreshExamStatusUI();
    $('adminConfigMsg').textContent = '✅ Exam is now OPEN for students.';
    setTimeout(() => { $('adminConfigMsg').textContent = ''; }, 3000);
  });

  $('adminCloseExamBtn').addEventListener('click', () => {
    const cfg = getExamConfig();
    cfg.isOpen = false;
    saveExamConfig(cfg);
    refreshExamStatusUI();
    $('adminConfigMsg').textContent = '🔒 Exam is now LOCKED.';
    setTimeout(() => { $('adminConfigMsg').textContent = ''; }, 3000);
  });
  ...
  $('adminSaveTasksBtn').addEventListener('click', () => {
    ...
    saveExamConfig(cfg);
    updateExamPreview();
    $('taskListMsg').textContent = '✅ Tasks saved!';
    setTimeout(() => { $('taskListMsg').textContent = ''; }, 2500);
  });

  $('adminClearTasksBtn').addEventListener('click', () => {
    if (!confirm('⚠️ Remove ALL tasks from the exam?')) return;
    pendingTasks = [];
    renderTaskList();
    const cfg = getExamConfig();
    cfg.tasks = [];
  ...
  document.querySelectorAll('[data-preset]').forEach((btn) => {
    btn.addEventListener('click', () => {
      ...
      renderTaskList();
      $('taskListMsg').textContent = '📦 Preset loaded. Click SAVE ALL TASKS to apply.';
  ...
  function refreshTokenUI() {
    ...
    const tokenInput = $('adminRetakeToken');
    const statusEl = $('adminTokenStatus');
    const expiryEl = $('adminTokenExpiry');

    if (!data) {
      tokenInput.value = '';
      statusEl.value = 'No active token';
    ...
    statusEl.style.color = '#6ee7a0';
    if (data.expires) {
      const d = new Date(data.expires);
      expiryEl.value = d.toLocaleString();
    } else {
      expiryEl.value = 'Never';
    }
  }
  ...
  $('adminGenerateToken').addEventListener('click', () => {
    ...
    localStorage.setItem('retake_token', JSON.stringify({
      token, expires, createdAt: Date.now()
    }));
    refreshTokenUI();
  ...
  $('adminCopyToken').addEventListener('click', () => {
    ...
    if (!token) {
      $('retakeTokenMsg').textContent = '⚠️ No token to copy. Generate one first.';
      return;
    }
    if (navigator.clipboard && navigator.clipboard.writeText) {
      navigator.clipboard.writeText(token).then(() => {
        $('retakeTokenMsg').textContent = '📋 Token copied to clipboard!';
  ...
  function fallbackCopy(token) {
    ...
    try {
      ...
      $('retakeTokenMsg').textContent = '⚠️ Copy failed. Manually copy: ' + token;
    }
    setTimeout(() => { $('retakeTokenMsg').textContent = ''; }, 3000);
  }
  ...
  $('adminTestSheet').addEventListener('click', async () => {
    const result = $('adminSheetTestResult');
    result.textContent = '📤 Sending test record...';
    result.className = 'pt-result';

    const cfg = getSheetConfig();
    if (!cfg.url || cfg.url.includes('YOUR_SCRIPT_ID')) {
      result.textContent = '❌ Configure the Webhook URL first.';
      result.className = 'pt-result error';
      return;
    }

    const res = await submitToGoogleSheet({
      studentName: 'TEST STUDENT', studentId: 'TEST-001',
      score: 100, passed: true, violations: 0,
      timestamp: new Date().toISOString(),
      challenges: 1, details: { test: true }
    });
    if (res.success) {
      result.textContent = '✅ Test sent! Check your Google Sheet.';
  ...
  function refreshResultsTable() {
    let history = [];
    try { history = JSON.parse(localStorage.getItem('exam_history') || '[]'); } catch (e) { history = []; }

    const total = history.length;
    const passed = history.filter((h) => h.passed).length;
    const failed = total - passed;
    const avgScore = total > 0 ? Math.round(history.reduce((a, h) => a + (h.score || 0), 0) / total) : 0;
  ...
  $('adminClearResults').addEventListener('click', () => {
    if (!confirm('Clear ALL LOCAL results? (Google Sheets not affected)')) return;
  ...
  window.addEventListener('storage', (e) => {
    if (e.key === 'exam_config' || e.key === 'exam_history' || e.key === 'retake_token') {
      refreshExamStatusUI();
      refreshResultsTable();
      refreshTokenUI();
    }
...
window.initStudentPage = function () {
  ...
  (function () {
    ...
    function buildPalette() {
      ...
      shuffledPalette.forEach((wire, idx) => {
        ...
        if (wire.stripe) {
          ...
          item.style.background = wire.hex;
        }
    ...
    function updateSlotVisual(slotIdx) {
      ...
      if (wire) {
        ...
        if (wire.stripe) {
          ...
          fill.style.background = wire.hex;
        }
    ...
    function placeWireInSlot(slotIdx, paletteIdx) {
      if (isCrimped) return;
      const wireTemplate = shuffledPalette[paletteIdx];
      if (!wireTemplate) return;
      slotWires[slotIdx] = { ...wireTemplate };
      updateSlotVisual(slotIdx);
      $('feedbackMsg').textContent = `Placed ${wireTemplate.name} in slot ${slotIdx + 1}`;
    ...
    function checkCable() {
      ...
      const results = [];
      for (let i = 0; i < 8; i++) {
        const userWire = slotWires[i];
        const stdWire = STANDARDS.B[i];
        const ok = userWire && userWire.name === stdWire.name;
        if (ok) correct++;
    ...
    function setCableType(type) {
      ...
      if (type === 'straight') {
        ...
        $('taskBanner').innerHTML = '<strong>TASK:</strong> Terminate a <strong>Crossover</strong> cable — this end T568B, other end T568A.';
      }
      resetAll();
    }
  ...
  (function () {
    ...
    $('examRefreshBtn').addEventListener('click', () => {
    ...
    function submitCrimpTask(t) {
      ...
      finishTask(t, earned, allCorrect,
        allCorrect ? 'Perfect crimp!' : `${correct}/8 wires correct`);
    }
    ...
    function createLab(canvasId, deviceTypes) {
      ...
      return {
        ...
        hoverDeviceId: null, dragDeviceId: null,
        dragOffsetX: 0, dragOffsetY: 0,
        connectSourceId: null, mouseX: null, mouseY: null,
        nextId: 1, deviceTypes
      };
    }

    function labDraw(lab) {
      const { ctx, canvas, devices, connections, tool, connectSourceId, hoverDeviceId } = lab;
    ...
    function setupExamLabToolbar(prefix, deviceTypes) {
    ...
    function loadTopoTask(t) {
      ...
      setupExamLabToolbar('exam', currentLab.deviceTypes);
    ...
    function submitTopoTask(t) {
      ...
      let msg = passed ? `Perfect! Detected: ${detected.toUpperCase()}`
    ...
    function submitSubnetTask(t) {
      ...
      finishTask(t, earned, allCorrect,
        allCorrect ? 'All subnet details correct!' : `${correct}/4 fields correct`);
    }
    ...
    function loadFirewallTask(t) {
      ...
      $('examTaskDesc').innerHTML =
        `<strong>${t.title}</strong> (${t.points} pts)<br>
        <span style="font-size:0.85rem; color:#8ba9bc;">
          Required rule: <strong>${t.requiredRule}</strong> · Build a topology with Firewall
        </span>`;
    ...
    async function endExam(autoSubmit, reason) {
      ...
      taskResults.forEach((r, i) => {
        ...
        row.innerHTML = `
          ...
          <span class="edr-info">
            ...
            <span style="font-size:0.75rem; color:#8ba9bc;">
              ${r.type} · ${r.message}
            </span>
          </span>
      ...
      if (sheetCfg.url && !sheetCfg.url.includes('YOUR_SCRIPT_ID') && sheetCfg.autoSync) {
        syncStatus.textContent = '☁️ Syncing to Google Sheets...';
        syncStatus.className = 'pt-result';
        const result = await submitToGoogleSheet(examRecord);
        if (result.success) {
          syncStatus.textContent = '✅ Record saved to Google Sheets.';
          syncStatus.className = 'pt-result ok';
        } else {
          syncStatus.textContent = '⚠️ Local save OK, cloud sync failed: ' + result.error;
          syncStatus.className = 'pt-result warn';
        }
      } else {
        syncStatus.textContent = 'ℹ️ Cloud sync not configured. Record saved locally.';
        syncStatus.className = 'pt-result warn';
      }
    }
    ...
    // ==================== RETAKE WITH TOKEN ====================
    const retakeBtn = $('examRetakeBtn');
    const retakeModal = $('retakeModal');
    const retakeInput = $('retakeTokenInput');
    const retakeError = $('retakeError');
    const retakeCancel = $('retakeCancelBtn');
    const retakeConfirm = $('retakeConfirmBtn');
    ...
    retakeBtn.addEventListener('click', () => {
      ...
      setTimeout(() => retakeInput.focus(), 100);
    });
    ...
    retakeInput.addEventListener('keypress', (e) => {
      if (e.key === 'Enter') retakeConfirm.click();
    });

    retakeConfirm.addEventListener('click', () => {
      const entered = (retakeInput.value || '').trim().toUpperCase();
      ...
      let stored = null;
      try {
        stored = JSON.parse(localStorage.getItem('retake_token') || 'null');
      } catch (e) { stored = null; }
      ...
      setTimeout(() => {
        ...
        // Refresh config display and show ready screen
        window.refreshExamScreen();
      }, 700);
    });
  ...
  (function () {
    ...
    let nextId = 1, mouseX = null, mouseY = null;
    ...
    function draw() {
      ...
      devices.forEach((d) => {
        ...
        if (!style) return;
        const sel = d.id === selectedDeviceId, hov = d.id === hoverDeviceId, cs = d.id === connectSourceId;
        ctx.beginPath();
        ctx.arc(d.x, d.y, 28, 0, Math.PI * 2);
        ctx.fillStyle = style.color;
        ctx.fill();
        ctx.strokeStyle = cs ? '#ffb347' : sel ? '#88c0d0' : '#2a4a5a';
        ctx.lineWidth = sel || cs ? 4 : 2;
        ctx.stroke();
        ctx.font = '24px "Segoe UI", system-ui, sans-serif';
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
